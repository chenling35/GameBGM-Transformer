...
- **铁律：所有内容不得擅自编造** — 代码、数据、注释、日志、文档、UI 文案，任何输出都必须基于真实情况。不确定的就先确认，没有的就如实说"暂无"，严禁为了应付任务而瞎编乱造
- **铁律：发现疑问先讨论再动手** — 当用户的要求与常规做法或已有逻辑矛盾时，不要死板执行。用户可能没想清楚，AI 也可能理解错。应先主动讨论、理清逻辑链条，达成共识后再写代码，减少返工
